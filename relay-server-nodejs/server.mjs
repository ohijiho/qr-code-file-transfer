...
const hostServer = new HostServer(configs.relayers, { logger });
...
app.post("/api/host/open", (req, res) => {
  try {
    const hid = hostServer.open(
      req.cookies[IDENTITY_KEY],
...
app.post("/api/host/accept/:hid", async (req, res) => {
  try {
    const d = await hostServer.accept(
      req.params.hid,
      req.cookies[IDENTITY_KEY],
    );
    if (d) {
...
app.get("/api/relay/:sid/closed", (req, res) => {
  try {
    const closed = relayServer.isClosed(
      req.params.sid,
      req.cookies[IDENTITY_KEY],
      res,
    );
...
if (
  !("httpPort" in configs) &&
  !("httpsPort" in configs && "httpsOptions" in configs)
) {
  throw new Error("neither http nor https is enabled in config");
}

if ("httpPort" in configs) {
  app.listen(configs.httpPort, () => {
    logger.info(`listening for HTTP at port ${configs.httpPort}`);
  });
}

if ("httpsPort" in configs && "httpsOptions" in configs) {
  https
    .createServer(configs.httpsOptions, app)
    .listen(configs.httpsPort, () => {
      logger.info(`listening for HTTPS at port ${configs.httpsPort}`);
    })
    .on("connection", (socket) => {
      logger.info(
        `Connection established: ${socket.remoteFamily} ${joinAddressAndPort(socket.remoteAddress, socket.remotePort)}`,
      );
    });
}
...
setInterval(() => {
  ...
  if (
    hostServer.dbgHostsOpen ||
    hostServer.dbgTotalListeners ||
    hostServer.dbgSocksOpen ||
    hostServer.dbgTotalUnfulfilled ||
    hostServer.dbgTotalQueued
  ) {
    ...
    if (hostServer.dbgTotalListeners)
      logger.info(
        ` - ${hostServer.dbgTotalListeners} listeners are currently waiting`,
      );
    ...
    if (relayServer.dbgTotalUnfulfilled)
      logger.info(
        ` - ${relayServer.dbgTotalUnfulfilled} relay requests are unfulfilled`,
      );
