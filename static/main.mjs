...
import {
  ByteSplitStream,
  ChunkwiseJSONParseStream,
  NullSeparatedJSONStringifyStream,
  ChecksumStream,
} from "./stream.mjs";
...
export class CipherSocket {
  ...
  start() {
    ...
    this.#closed = Promise.all([this.#sendLoopPromise, this.#recvLoopPromise]);
  }
    ...
    const ts = new TransformStream({
      ...
      async flush(controller) {
        const [a, b] = await Promise.all([
          cp.then(uint8Array),
          base64ToBytes(await d.checksum),
        ]);
        if (!taeq(a, b)) {
          const err = new Error(
            `wrong checksum: '${bytesToBase64(a)}' != '${bytesToBase64(b)}'`,
          );
