    ...
    return new TransformStream({
      ...
      transform: async (chunk, controller) => {
        ...
        for (let i = 0; i < chunk.byteLength; i += chunkSize) {
          const d = await this.encrypt(
            new Uint8Array(
              chunk.buffer,
              chunk.byteOffset + i,
              Math.min(chunkSize, chunk.byteLength - i),
            ),
          );
      ...
      transform: async (chunk, controller) => {
        ...
        while (remaining <= 0) {
          if (len === null) {
            ...
            if (buf[0].byteLength >= 2) {
              // buf.len = 1
              len = new DataView(
                new Uint8Array([buf[0][0], buf[0][1]]).buffer,
              ).getUint16(0);
              buf[0] = new Uint8Array(buf[0].buffer, buf[0].byteOffset + 2);
            } else {
              // buf.len = 2
              len = new DataView(
                new Uint8Array([buf[0][0], buf[1][0]]).buffer,
              ).getUint16(0);
